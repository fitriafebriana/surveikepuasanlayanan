@extends('layouts.dashboard', [
  'breadcrumbs' => [
      'Kuesioner' => route('kuisioner.index'),
      'Edit Kuesioner' => '#'
  ],
])
...
@section('content')
  <x-card>
    <div class="relative overflow-x-auto sm:rounded-lg p-5">
      <form action="{{ route('kuisioner.update', $kuisioner->id) }}" method="POST">
        @method('PUT')
        @csrf
        {{-- <div class="mb-3">
					<select name="unsur_id" class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
						...
						@foreach ($unsurs as $unsur)
							<option value="{{ $unsur->id }}" {{ $kuisioner->unsur->unsur == $unsur->unsur ? 'selected' : '' }}>{{ $unsur->unsur }}</option>
						@endforeach
					</select>
				</div> --}}
        <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
          ...
          <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
            ...
            <textarea id="editor" rows="8"
              ...
              placeholder="Tulis pertanyaan kuisioner...">{{ $kuisioner->question }}</textarea>
          </div>
        </div>
        <div class="form-check form-switch">
          <input class="form-check-input" type="checkbox" value="true" id="aktif" name="aktif"
              @if ($kuisioner->aktif) checked @endif>
